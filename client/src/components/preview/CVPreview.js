    ...
    >
      <div className="p-8">
              ...
              {skillCategories.map(category => 
                cvData.skills[category.key].length > 0 && (
                  <div key={category.key} className="mb-2">
                    ...
                    <span>{cvData.skills[category.key].join(', ')}</span>
                  </div>
                )
              )}
            </div>
        ...
        {cvData.experience.length > 0 && (
          <CVSection title="Professional Experience">
            {cvData.experience.map((exp, index) => (
              <CVExperienceItem key={index} experience={exp} />
            ))}
          </CVSection>
        )}
        ...
        {cvData.projects.length > 0 && (
          <CVSection title="Key Projects">
            {cvData.projects.map((project, index) => (
              <CVProjectItem key={index} project={project} />
            ))}
          </CVSection>
        )}
          ...
          <CVSection title="Education & Awards">
            {cvData.education.degree && (
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-3">
                <div className="font-medium">
                  <span className="font-bold">{cvData.education.degree}</span>
                  {cvData.education.university && (
                    <span className="ml-2">| {cvData.education.university}</span>
                  )}
                </div>
                {cvData.education.duration && (
                  <span className="italic text-sm mt-1 sm:mt-0" style={{ fontSize: '10pt' }}>
                    {cvData.education.duration}
                  </span>
                )}
              </div>
            ...
            {cvData.awards.filter(award => award.trim()).length > 0 && (
              <div className="mt-3">
                ...
                <span>{cvData.awards.filter(award => award.trim()).join(' | ')}</span>
              </div>
            )}
          </CVSection>
        )}
        ...
        {!cvData.personalInfo.name && !cvData.summary && 
         Object.values(cvData.skills).every(skills => skills.length === 0) && 
         cvData.experience.length === 0 && cvData.projects.length === 0 && 
         !cvData.education.degree && cvData.awards.length === 0 && (
          <div className="text-center py-20 text-gray-500">
            <p className="text-lg mb-2">Your CV preview will appear here</p>
...
const CVHeader = ({ personalInfo }) => (
  <div className="text-center mb-6 pb-4 border-b border-gray-300">
    ...
    <div className="text-sm text-gray-600 flex flex-wrap justify-center gap-2" style={{ fontSize: '10pt' }}>
      {[
        personalInfo.phone,
        personalInfo.email,
  ...
  <div className="mb-4">
    <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-2">
      <div className="font-medium">
        ...
        {experience.company && (
          <span className="ml-2">| <span className="font-bold">{experience.company}</span></span>
        )}
      </div>
      {experience.duration && (
        <span className="italic text-sm mt-1 sm:mt-0" style={{ fontSize: '10pt' }}>
    ...
    {experience.bullets.filter(bullet => bullet.trim()).length > 0 && (
      <ul className="ml-4 space-y-1">
        {experience.bullets.filter(bullet => bullet.trim()).map((bullet, bulletIndex) => (
          <li key={bulletIndex} className="list-disc text-justify">
            {bullet}
          </li>
        ))}
      </ul>
...
const CVProjectItem = ({ project }) => (
  <div className="mb-4">
    ...
    {project.techStack && (
      <div className="italic text-gray-600 text-sm mb-2" style={{ fontSize: '10pt' }}>
        {project.techStack}
      </div>
    )}
