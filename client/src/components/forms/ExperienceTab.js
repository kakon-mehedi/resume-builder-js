...
const ExperienceTab = ({ experience, addExperience, updateExperience, removeExperience, setCvData }) => {
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-semibold text-gray-800">Work Experience</h3>
        <Button onClick={addExperience} icon={<Plus size={16} />} variant="success">
          Add Experience
        </Button>
      </div>
      ...
      {experience.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg">
          ...
          <Button onClick={addExperience} icon={<Plus size={16} />}>
            Add Your First Job
          </Button>
        </div>
      ) : (
...
const ExperienceItem = ({ experience, index, onUpdate, onRemove, setCvData }) => {
  const addBullet = () => {
    setCvData(prev => ({
      ...prev,
